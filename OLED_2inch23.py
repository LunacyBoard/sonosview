...
class OLED_2inch23(framebuf.FrameBuffer):
    ...
    def fill_screen (self, row1 = "", row2 = "", row3 = "", progress = 0):
        """Clear the screen and populate rows 1-3 with values
        """
        self.fill(0x0000)
        self.text(row1,1,0,self.white)
        self.text(row2,1,10,self.white)        
        self.text(row3,1,20,self.white)
        if progress == 0:
            self.rect(0,29,128,3,self.black)
        else:
            self.rect(0,29,int(128*progress),3,self.white)
        self.show()
        
    def scroller (self, strScroll):
        """ Scroll text wider than 16 chars
        """
        if len(strScroll) > 16:
            return strScroll[1:] + strScroll[0]
        else:
            return strScroll
...
if __name__=='__main__':
    ...
    OLED.text("Pico-OLED-2.23p",1,12,OLED.white)
